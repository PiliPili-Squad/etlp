export const zhCN = {
    ...
    sys_liquid_glass: "液态玻璃（实验性）",
    sys_liquid_glass_desc: "启用 macOS 26 的液态玻璃窗口材质；修改后需重启应用生效",
    sys_liquid_glass_unavailable: "当前系统不支持液态玻璃，已禁用",
    sys_display: "显示",
    sys_font_size: "字体大小",
    ...
    cfg_restore_progress_title: "正在还原配置",
    cfg_restore_progress_message:
        "配置正在写入并应用到本地服务。完成前请保持应用运行，避免退出或强制关闭。",
    cfg_restore_progress_label: "正在应用配置 {progress}%",
    cfg_restore_progress_waiting: "正在完成最后的配置应用…",
    cfg_restore_failed_title: "还原失败",
    cfg_restore_error_ack: "知道了",
    cfg_restore_error_read: "无法读取备份文件，请确认文件仍然存在且有访问权限。",
    cfg_restore_error_zip: "备份文件无法解析，请确认选择的是有效的配置备份 zip。",
    cfg_restore_error_missing_config: "备份文件缺少配置内容，请重新选择正确的备份。",
    cfg_restore_error_invalid_config: "备份中的配置格式不正确，当前配置未被覆盖。",
    cfg_restore_error_write: "配置写入失败，请检查磁盘空间和文件权限后重试。",
    cfg_restore_error_apply: "配置已写入，但应用到本地服务失败，请重启应用后检查配置。",
    cfg_restore_error_internal: "还原任务异常中断，请重启应用后再试。",
    cfg_restore_error_generic: "配置还原失败，请稍后重试。",
